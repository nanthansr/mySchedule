// The 3D shelf engine - plain Three.js, no framework coupling. Adapted from
// mint-playground (github.com/mintdotgg/mint-playground
// @ 331533abd5f74acebb36b3598e5c811e2f038901, MIT - see
// licenses/mint-playground.LICENSE). Changes from upstream: the optional
// Stripe-Press asset archive (OBJ mesh + harvested textures, not covered by
// the MIT grant) is removed entirely - covers are always procedural - and
// the room is recolored to this site's dark identity.
...
export class ShelfEngine {
  ...
  private setupScene() {
    ...
    this.scene.fog = new THREE.Fog("#0b0b0e", 10, 26);

    const hemisphere = new THREE.HemisphereLight("#dfe7f5", "#2a2118", 2.0);
